...
from ..providers.rtquote import credentials_present, quote_token, resolve_symbols
...
def write_by_day(symbol: str, df: pd.DataFrame, min_bars: int = 2) -> list[str]:
    ...
    for day, part in df.groupby(df["timestamp"].dt.date):
        if len(part) < min_bars:
            log.debug("%s %s: %d isolated bar(s), skipped",
                      symbol, day, len(part))
            continue
    ...
    async with websockets.connect(url, max_size=2 ** 24) as ws:
        ...
        while True:
            try:
                ...
                log.info("Connection closed by dxFeed — %d symbols received",
                         len(rows))
                break
            m = json.loads(raw)
            typ = m.get("type")
            if typ == "AUTH_STATE":
                ...
                if state == "UNAUTHORIZED" and not auth_sent:
                    ...
                    # HISTORY contract: enables fromTime queries
                ...
                log.info("History requested: %d symbols from %s",
...
def backfill(symbols: list[str] | None = None, days: int = 45,
    ...
    if not credentials_present():
        log.error("tastytrade credentials missing — backfill not possible")
        return {}
    ...
    if symbols:
        ...
        if missing:
            log.warning("Unknown symbols, skipped: %s", ", ".join(missing))
    ...
    for key, df in sorted(frames.items()):
        if df.empty:
            log.warning("%s: no candles received", key)
            continue
        days_written = write_by_day(key, df)
        counts[key] = len(df)
        log.info("%-6s %5d bars  %s → %s  (%d sessions)", key, len(df),
                 df["timestamp"].min().date(), df["timestamp"].max().date(),
                 len(days_written))
        if len(df) >= OBSERVED_CAP:
            log.info("%s: cap reached — history likely extends further; rerun with --period 5m to cover more days", key)
    return counts
...
def main() -> None:
    ...
    parser.add_argument("--days", type=int, default=45,
                        help="look-back depth (default: 45)")
    parser.add_argument("--period", default="m",
                        help="dxFeed granularity: m, 5m, 15m, h… (default: m)")
    parser.add_argument("--symbols", nargs="*",
                        help=f"subset; default: all "
                             f"({', '.join(sorted(UNDERLYINGS))} + futures)")
    args = parser.parse_args()
    logging.basicConfig(level=logging.INFO, format="%(message)s")
    counts = backfill(args.symbols, args.days, args.period)
    print(f"\n{sum(counts.values())} bars written for {len(counts)} symbols.")
