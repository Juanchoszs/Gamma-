...
from gex.domain.options import greeks
from gex.adapters.external import rates
from gex.infrastructure.config import CONTRACT_MULTIPLIER, SETTINGS
...
def key_levels(df: pd.DataFrame, spot: float,
               ref_spot: float | None = None,
               all_expiries: bool = False) -> dict[str, object]:
    ...
    """
    out: dict[str, object] = {
        "call_wall": None, "put_support": None,
        "call_walls": [], "put_supports": [],
        "d1_min": None, "d1_max": None,
    }
    if df.empty:
        return out
    nearest = df["expiry"].min()
    ...
    if len(above):
        call_agg = gex_at_spot(sub[sub["type"] == "C"], ref_spot) if ref_spot else (
            sub[sub["type"] == "C"].groupby("strike")["gex"].sum()
        )
        call_levels = call_agg[(call_agg.index >= spot) & (call_agg > 0)].loc[
            call_agg[(call_agg.index >= spot) & (call_agg > 0)].abs().nlargest(3).index
        ].sort_values(
            key=abs, ascending=False,
        )
        out["call_walls"] = [float(strike) for strike in call_levels.index]
        out["call_wall"] = float(call_levels.index[0])
    below = agg[(agg.index <= spot) & (agg < 0)]
    if len(below):
        put_agg = gex_at_spot(sub[sub["type"] == "P"], ref_spot) if ref_spot else (
            sub[sub["type"] == "P"].groupby("strike")["gex"].sum()
        )
        put_levels = put_agg[(put_agg.index <= spot) & (put_agg < 0)].loc[
            put_agg[(put_agg.index <= spot) & (put_agg < 0)].abs().nlargest(3).index
        ].sort_values(
            key=abs, ascending=False,
        )
        out["put_supports"] = [float(strike) for strike in put_levels.index]
        out["put_support"] = float(put_levels.index[0])
