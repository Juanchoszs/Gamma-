...
def window_metrics(df: pd.DataFrame, split_ts: float | None = None) -> dict:
    ...
    if split_ts is not None:
        ...
        if len(post):
            ...
            out["post_max_up"] = _r2(post.max() - c)     # upward excursion post-close
            out["post_max_down"] = _r2(c - post.min())   # downward excursion post-close
...
def stop_swept(df: pd.DataFrame, entry_price: float, stop_pts: float,
    ...
    p = d["price"]
    if direction >= 0:                     # long: stop below
