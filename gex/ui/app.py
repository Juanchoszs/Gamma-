...
from gex.providers.rtquote import PUBLIC_QUOTES, QUOTES, credentials_present
from ..application.scheduler import STATE, market_is_open
from ..application.scheduler import native_index_key as scheduler_native_key
from . import scales
...
def _app_fn(name: str):
    """Resolve callables via the gex.app facade so monkeypatches apply."""
    import gex.app as _mod
...
# Local timezone for all time axes
...
# Chart titles link to guide anchors on GitHub via Plotly's subset of HTML.
#
# Anchors are set EXPLICITLY in .md to prevent title translations from breaking links.
...
def exposure_fig(df: pd.DataFrame, spot: float, zg: float | None, col: str, title: str,
    ...
    # Transpose prices to selected scale
    xf = xf or (lambda v: v)
...
def heatmap_fig(symbol: str, lang: str, day: str | None = None,
    ...
    fig = go.Figure()
    # Thick OI bars behind thin volume bars
    for serie, name, width, colors in (
        ...
        if serie.empty:
            continue
        v = serie.to_numpy()
        # Rightward bars colored by sign
        fig.add_bar(
    ...
    # Horizontal markers
    # Spot reference always shown
    # Walls relative to structural spot
...
def _transform_for(symbol: str, scale_key: str | None):
    ...
    #
    #
    # Keep last basis when market closed
    if _app_fn("market_is_open")():
...
def regime_banner(symbol: str, lang: str) -> html.Div:
    ...
    if s is None or s.zero_gamma is None:
        return html.Div(style={"display": "none"})
    # Match Discord bot reading text
    hist = store.load_history(symbol)
...
def chain_state(symbol: str):
    ...
    """
    if symbol in idxopt.NATIVE_INDEX and _app_fn("credentials_present")():
...
def build_cards(symbol: str, lang: str, xf=None, scale: str | None = None) -> list:
    ...
    if s is None:
        ...
        if symbol in ("NQ", "ES") and not _app_fn("credentials_present")():
            # Never show collecting if no account exists
            # Show delayed or generic state
            # Reason shown via native_notice overlay
    ...
    # GEX heavily depends on spot
    # Regime reading requires fresh spot
    # Recalculate with realtime spot if available
    ...
    return [
        # Show real future price on future scale
        # Transposed index spot
...
card(t(lang, "card_net_gex"), f"{net_gex / 1e9:+.1f} $Bn",
              ...
              accent=gex_color),
        card(t(lang, "card_net_dex"), f"{s.net_dex / 1e9:+.1f} $Bn" if s.net_dex is not None else "—",
              t(lang, "dex_long") if (s.net_dex or 0) >= 0 else t(lang, "dex_short"),
              accent=C["pos"] if (s.net_dex or 0) >= 0 else C["neg"]),
        card(t(lang, "card_zero_gamma"), zg_txt, zg_sub, accent=C["zg"]),
        card(t(lang, "card_gex_0dte"), f"{s.net_gex_0dte / 1e9:+.1f} $Bn" if s.net_gex_0dte is not None else "—"),
        card(t(lang, "card_pc_oi"), f"{s.pc_oi:.2f}"),
        card(t(lang, "card_pc_vol"), f"{s.pc_volume:.2f}"),
    ]


# Central dispatch for exporting any dashboard chart to PNG.
...
def _figure_for(symbol: str, name: str, lang: str = "fr", bucket: str = "Tout",
    ...
    if bucket not in BUCKET_KEYS:
        bucket = "Tout"
    win = window if window is not None else 0.04
    today = datetime.now(ET).strftime("%Y-%m-%d")
    today_d = datetime.now(ET).date()
    # Target scale (e.g., NDX GEX shown in NQ prices).
    ...
    # Charts needing current enriched chain
    st = chain_state(symbol)
    with STATE.lock:
        df, snap = st.enriched, st.snapshot
    ...
    live = spot if _app_fn("market_is_open")() else structural
...
def chart_png(symbol: str, name: str, lang: str = "fr", bucket: str = "Tout",
    ...
    # Plotly JSON round-trip for timestamps
...
def create_app() -> Dash:
    ...
    app.layout = html.Div([
        # Overlay for missing credentials
        # Notice content populated by callback
        ...
        html.Div([
            ...
            html.Div(id="pane-heat", children=[
                ...
                html.Div([
                    # Available days from chain snapshots
    ...
    def levels_strip(levels: pd.DataFrame | None, lang: str,
        ...
        if hvl is not None:
            items.append(_chip([html.B("HVL ", style={"color": C["hvl"]}),
                                f"{xf(hvl):.0f}"], C["hvl"]))
        # Directional levels and expected move
        for key, color, label in (("call_wall", C["cw"], "Call Wall"),
                                  ("put_support", C["ps"], "Put Support"),
                                  ("d1_min", C["d1"], "1D Min"),
                                  ("d1_max", C["d1"], "1D Max")):
    ...
    # Language choice overrides browser locale
    ...
    def rt_status(_, lang):
        ...
        """
        # Show connect button until account is usable
        ...
        if etat_tt == "connecte":
            ...
            # Show info without broken link
            bouton = ({"display": "none"}, "", detail_tt)

        state, detail = QUOTES.status(market_open=_app_fn("market_is_open")())
        if state == "off":
        ...
        # Subtitle reflects realtime status
    ...
    def native_notice(symbol, lang):
        ...
        hidden = {"display": "none"}
        if symbol not in ("NQ", "ES") or _app_fn("credentials_present")() or PUBLIC_QUOTES.price(symbol):
    ...
    def refresh(_, symbol, bucket, window, majors, flow_day, lang, unit, gflow_series,
        ...
        if df is None or snap is None:
            # Fallback to most recent non-empty historical snapshot
            hist_days = store.snapshot_days(symbol)
            for day in reversed(hist_days):
                hist_df = store.load_last_snapshot(symbol, day)
        ...
        side_spot = snap.spot if _app_fn("market_is_open")() else ref
        # Structural spot
    ...
    def refresh_heatmap(_, tab, symbol, window, lang, unit, day, levels_shown, relayout):
        # Skip reads if tab hidden
        if tab != "heat":
            raise PreventUpdate
        # Reset zoom on context change
    ...
    def refresh_tape(_, tab, symbol, min_size, combos, lang):
        # Only recalculate on visible tab
        if tab != "tape":
            raise PreventUpdate
    ...
    def back_to_today(_, symbol):
        today = datetime.now(ET).strftime("%Y-%m-%d")
        days = available_flow_days(symbol)
        # Current day if flows exist, else most recent
