...
_QUOTE_TOKEN_LOCK = threading.Lock()
_QUOTE_TOKEN_CACHE: tuple[float, tuple[str, str, str]] | None = None
_QUOTE_TOKEN_CACHE_S = 120.0
...
def _is_real_val(v: str | None) -> bool:
    ...
    v_low = v_clean.lower()
    return bool(v_clean) and not (
        v_low.startswith("pega_aqui")
...
def _env(name: str) -> str | None:
    ...
    # 2. Variable os.environ
    val = os.environ.get(name)
    if _is_real_val(val):
        return val.strip().strip('"').strip("'")

    # 3. Repli registre Windows HKCU
    if sys.platform == "win32":
        ...
        try:
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment") as k:
                ...
                if _is_real_val(val):
                    cand = str(val).strip().strip('"').strip("'")
                    os.environ[name] = cand
                    return cand
        except OSError:
            pass
...
def _quote_token_uncached() -> tuple[str, str, str]:
...
def quote_token() -> tuple[str, str, str]:
    """Return shared credentials without racing the refresh-token exchange."""
    global _QUOTE_TOKEN_CACHE
    dx_direct = _env("DXFEED_AUTH_TOKEN")
    if _is_real_val(dx_direct):
        url = _env("DXFEED_ENDPOINT") or "wss://live.dxfeed.com/live/websocket"
        return dx_direct, url, ""

    # QUOTES, TAPE and CAPTURE start together. Reusing the short-lived result
    # prevents a rotating refresh token from being consumed concurrently.
    with _QUOTE_TOKEN_LOCK:
        now = time.monotonic()
        if _QUOTE_TOKEN_CACHE and _QUOTE_TOKEN_CACHE[0] > now:
            return _QUOTE_TOKEN_CACHE[1]
        result = _quote_token_uncached()
        _QUOTE_TOKEN_CACHE = (time.monotonic() + _QUOTE_TOKEN_CACHE_S, result)
        return result
...
class RealtimeQuotes:
    ...
    def status(self, market_open: bool = True) -> tuple[str, str]:
        ...
        """
        if credentials_present() and not self._started:
            self.start()
        if self._state == "off":
            return "off", ""
