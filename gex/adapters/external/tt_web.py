...
def connection_status() -> tuple[str, str]:
    ...
    from gex.adapters.market_data.rtquote import _env, _is_real_val
...
def register_oauth(app) -> None:
    ...
    def _api_exchange():
        ...
        if not cid or not secret or not code:
            return jsonify({"ok": False, "error": "Client ID, Client Secret y Código de autorización son obligatorios."}), 400

        try:
            token_data = tt_auth.exchange_code(cid, secret, code)
    ...
    def _oauth_callback():
        ...
        try:
            ...
            return _page("Error de conexión",
                         f"No fue posible contactar a los servidores de Tastytrade ({exc}). "
...
def _demarrer_les_flux() -> None:
    ...
    try:
        if credentials_present():
            ...
            from gex.adapters.market_data.rtquote import QUOTES
